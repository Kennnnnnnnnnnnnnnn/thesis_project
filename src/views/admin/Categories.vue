<template>
  <div class="p-4 md:p-6 bg-gray-50 min-h-screen ">
        ...
        <div class="overflow-x-auto w-full">
          <table class="min-w-full divide-y divide-gray-100">
            ...
            <tbody class="bg-white divide-y divide-gray-100">
              <tr v-for="(item, index) in categoryData" :key="item._id" class="hover:bg-amber-50/50 transition-colors duration-200">
                ...
                <td class="px-6 py-4 whitespace-nowrap text-center">
                  <div class="text-sm text-gray-600 max-w-xs truncate">{{ item.description || 'No description' }}</div>
                </td>
                <td class="px-6 py-4 whitespace-nowrap text-center">
                ...
                <td class="px-6 py-4 whitespace-nowrap text-center">
                  <div class="flex items-center justify-center gap-2">
                    <button class="p-2.5 rounded-xl hover:bg-amber-50 text-amber-600 transition-all duration-200 hover:scale-110 border border-transparent hover:border-amber-200" @click="editCategory(item)" title="Edit category">
                      <i class="fas fa-edit text-sm"></i>
                    </button>
                    <button class="p-2.5 rounded-xl hover:bg-red-50 text-red-600 transition-all duration-200 hover:scale-110 border border-transparent hover:border-red-200" @click="deleteCategory(item._id)" title="Delete category">
              ...
              <tr v-if="categoryData.length === 0 && !isLoading">
                <td colspan="6" class="px-6 py-20 text-center">
                  <div class="flex flex-col items-center gap-4">
                    ...
                    <button @click="openModal" class="inline-flex items-center gap-2 px-6 py-3 bg-amber-500 text-white rounded-xl text-sm font-semibold hover:bg-amber-600 transition-all shadow-lg hover:shadow-xl transform hover:scale-105">
                      <i class="fas fa-plus text-xs"></i>
    ...
    <div v-if="showModal" class="fixed inset-0 flex items-center justify-center bg-black/60 backdrop-blur-sm z-[1000] p-4">
      <div class="bg-white rounded-3xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto border border-gray-200 p-4 sm:p-6">
        <!-- Modal Header -->
        <div class="flex items-center justify-between mb-6">
          ...
          <button class="p-2.5 rounded-xl hover:bg-gray-100 text-gray-400 hover:text-gray-600 transition-all" @click="closeModal">
        ...
        <form @submit.prevent="handleSubmit" class="space-y-6">
          <!-- Category Name -->
          <div>
            <label class="block text-sm font-bold text-gray-700 mb-3">Category Name <span class="text-red-500">*</span></label>
          ...
          <div class="flex flex-col sm:flex-row gap-4 pt-4">
            <button type="button" class="w-full sm:w-auto px-6 py-3.5 text-gray-700 bg-gray-100 rounded-2xl hover:bg-gray-200 font-semibold transition-all" @click="resetForm">
              Clear
            </button>
            <button type="submit" :disabled="isSubmitting"
              class="w-full sm:w-auto px-6 py-3.5 bg-gradient-to-r from-amber-500 to-orange-600 text-white rounded-2xl hover:from-amber-600 hover:to-orange-700 disabled:opacity-50 disabled:cursor-not-allowed font-semibold transition-all shadow-lg hover:shadow-xl transform hover:scale-[1.02]">
