        ...
        <li v-for="item in dynamicNavItems" :key="item.name">
          <router-link @click="closeMobileMenu" :to="item.route"
            ...
            <span class="flex items-center gap-3">
              <svg v-if="item.icon" class="w-5 h-5 text-yellow-500" fill="none" stroke="currentColor"
                viewBox="0 0 24 24">
                <use :href="`#${item.icon}`" />
              </svg>
              {{ $t(item.name) }}
            ...
            <span v-if="item.badge" class="px-2 py-1 bg-red-500 text-white text-xs font-bold rounded-full">{{ item.badge
              }}</span>
          </router-link>
        </li>
        <!-- Logout Button -->
